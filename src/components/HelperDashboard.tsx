...
import { ArrowLeft, Plus, MapPin, Phone, FileText, Image, Clock, Upload, Loader, Download } from 'lucide-react';
...
export function HelperDashboard({ onBack, onSubmitRequest, requests }: HelperDashboardProps) {
  ...
  const handleDownloadPDF = (request: RescueRequest) => {
    ...
    doc.text('RESCUE OPERATION DETAILS', 105, 20, { align: 'center' });
    ...
    doc.text(`Case ID: #${request.id}`, 20, y);
    y += 7;
    doc.text(`Status: ${getStatusText(request.status)}`, 20, y);
    y += 7;
    doc.text(`Date & Time: ${formattedDate}`, 20, y);
    y += 12;
    
    // Helper Information Section
    doc.setFont('helvetica', 'bold');
    doc.text('HELPER INFORMATION', 20, y);
    doc.setFont('helvetica', 'normal');
    y += 7;
    doc.text(`Name: ${request.helperName}`, 20, y);
    y += 7;
    doc.text(`Phone: ${request.helperPhone}`, 20, y);
    y += 7;
    if (request.helperAltPhone) {
      doc.text(`Alt. Phone: ${request.helperAltPhone}`, 20, y);
      y += 7;
    }
    if (request.helperEmail) {
      doc.text(`Email: ${request.helperEmail}`, 20, y);
      y += 7;
    }
    y += 5;
    ...
    y += 7;
    const locationLines = doc.splitTextToSize(request.location, 170);
    doc.text(locationLines, 20, y);
    y += (locationLines.length * 7) + 7;
    
    doc.text('Description:', 20, y);
    y += 7;
    const descriptionLines = doc.splitTextToSize(request.notes, 170);
    doc.text(descriptionLines, 20, y);
    y += (descriptionLines.length * 7) + 5;
    
    // Rescue Team Section
    doc.setFont('helvetica', 'bold');
    doc.text('RESCUE TEAM', 20, y);
    doc.setFont('helvetica', 'normal');
    y += 7;
    
    if (request.assignedRescuer) {
      doc.text(`Assigned Rescuer: ${request.assignedRescuer}`, 20, y);
      y += 7;
    } else {
      doc.text('No rescuer assigned', 20, y);
      y += 7;
    }
    
    if (request.trackingId) {
      doc.text(`Tracking ID: ${request.trackingId}`, 20, y);
      y += 7;
    }
    
    // Rescue Summary Section
    if (request.rescuerNotes) {
      y += 5;
      doc.setFont('helvetica', 'bold');
      doc.text('RESCUE SUMMARY', 20, y);
      doc.setFont('helvetica', 'normal');
      y += 7;
      const notesLines = doc.splitTextToSize(request.rescuerNotes, 170);
    ...
    doc.text('Generated by Hope Spot Rescue Platform', 105, y, { align: 'center' });
    y += 5;
    doc.text(generatedDate, 105, y, { align: 'center' });
    
    doc.save(`rescue-case-${request.id}-${Date.now()}.pdf`);
  };
                ...
                <div key={request.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="pb-3 border-b border-gray-200">
                      <h4 className="text-gray-700 mb-2">Helper Information</h4>
                  ...
                  {request.status === 'completed' && (
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      ...
                      >
                        ...
                        <span>Download Rescue Details (PDF)</span>
                      </button>
                    </div>
                  )}
                </div>
